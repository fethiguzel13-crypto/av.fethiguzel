/**
 * Tüm aktif fakülteler için Adli Tıp 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-adli-tip-triple.mjs
 */
...
import {
  ADLI_TIP_VARIANTS,
  buildAdliTipVariantNote,
} from './lib/adli-tip-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of ADLI_TIP_VARIANTS) {
    const note = buildAdliTipVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(ADLI_TIP_VARIANTS);
...
index.adliTipTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'adli-tip-donem-1',
        title: 'Adli Tıp — 1. Dönem (Güz)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/adli-tip-donem-1`,
        ready: true,
        variantOf: 'adli-tip',
      },
      {
        code: 'adli-tip-donem-2',
        title: 'Adli Tıp — 2. Dönem (Bahar)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/adli-tip-donem-2`,
        ready: true,
        variantOf: 'adli-tip',
      },
      {
        code: 'adli-tip-yillik',
        title: 'Adli Tıp — Yıllık Tam Not',
        year: 3,
        href: `/ders-notlari/${uniSlug}/adli-tip-yillik`,
        ready: true,
        variantOf: 'adli-tip',
      },
    ];
...
console.log(
  `[adli-tip-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${ADLI_TIP_VARIANTS.join(',')}`
);
console.log(`[adli-tip-triple] index notes now=${index.noteCount}`);
