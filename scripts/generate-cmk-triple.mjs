/**
 * Tüm aktif fakülteler için CMK 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-cmk-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { CMK_VARIANTS, buildCmkVariantNote } from './lib/cmk-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of CMK_VARIANTS) {
    const note = buildCmkVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(CMK_VARIANTS);
...
index.cmkTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'cmk-donem-1',
        title: 'CMK — 1. Dönem (Güz)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/cmk-donem-1`,
        ready: true,
        variantOf: 'cmk',
      },
      {
        code: 'cmk-donem-2',
        title: 'CMK — 2. Dönem (Bahar)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/cmk-donem-2`,
        ready: true,
        variantOf: 'cmk',
      },
      {
        code: 'cmk-yillik',
        title: 'CMK — Yıllık Tam Not',
        year: 3,
        href: `/ders-notlari/${uniSlug}/cmk-yillik`,
        ready: true,
        variantOf: 'cmk',
      },
    ];
...
console.log(
  `[cmk-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${CMK_VARIANTS.join(',')}`
);
console.log(`[cmk-triple] index notes now=${index.noteCount}`);
