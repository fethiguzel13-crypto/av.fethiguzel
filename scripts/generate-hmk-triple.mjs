/**
 * Tüm aktif fakülteler için HMK 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-hmk-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { HMK_VARIANTS, buildHmkVariantNote } from './lib/hmk-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of HMK_VARIANTS) {
    const note = buildHmkVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(HMK_VARIANTS);
...
index.hmkTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'hmk-donem-1',
        title: 'HMK — 1. Dönem (Güz)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/hmk-donem-1`,
        ready: true,
        variantOf: 'hmk',
      },
      {
        code: 'hmk-donem-2',
        title: 'HMK — 2. Dönem (Bahar)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/hmk-donem-2`,
        ready: true,
        variantOf: 'hmk',
      },
      {
        code: 'hmk-yillik',
        title: 'HMK — Yıllık Tam Not',
        year: 3,
        href: `/ders-notlari/${uniSlug}/hmk-yillik`,
        ready: true,
        variantOf: 'hmk',
      },
    ];
...
console.log(
  `[hmk-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${HMK_VARIANTS.join(',')}`
);
console.log(`[hmk-triple] index notes now=${index.noteCount}`);
