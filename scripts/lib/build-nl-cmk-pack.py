...
OUT = ROOT / "docs" / "ders-notlari" / "notebooklm" / "dalga-a-ceza-muhakemesi"
GRAPH = ROOT / "lib" / "ders-notlari" / "graphs" / "ceza-muhakemesi.json"
...
def main() -> None:
    ...
    parts = ["# Ceza muhakemesi — resmi lafız (FSEK m. 31)\n", "Şerh yoktur. CMK.\n"]
    seen = set()
    ...
    (OUT / "harita.md").write_text(
        """# Dalga A — Ceza muhakemesi

Quiz CMK lafzı ve graf unsura dayanır. Künye uydurulmaz. Slayt yoktur.

Kurum id: sorusturma, koruma, ifade-sorgu, kovusturma, delil-cmk, hukum-yol.

Tuzaklar: soruşturma/kovuşturma; ifade/sorgu; tutuklama/adli kontrol; KYOK/iddianame; yasak delil; istinaf/temyiz.

Fakülte sesi overlay'dedir. Ceza genel ve ceza özel ayrı derstir.
""",
        encoding="utf-8",
    )
