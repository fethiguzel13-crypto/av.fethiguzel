/**
 * Tüm aktif fakülteler için Hukuk Felsefesi 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-hukuk-felsefesi-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  HUKUK_FELSEFESI_VARIANTS,
  buildHukukFelsefesiVariantNote,
} from './lib/hukuk-felsefesi-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of HUKUK_FELSEFESI_VARIANTS) {
    const note = buildHukukFelsefesiVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(HUKUK_FELSEFESI_VARIANTS);
...
index.hukukFelsefesiTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'hukuk-felsefesi-donem-1',
        title: 'Hukuk Felsefesi — 1. Dönem (Güz)',
        year: 1,
        href: `/ders-notlari/${uniSlug}/hukuk-felsefesi-donem-1`,
        ready: true,
        variantOf: 'hukuk-felsefesi',
      },
      {
        code: 'hukuk-felsefesi-donem-2',
        title: 'Hukuk Felsefesi — 2. Dönem (Bahar)',
        year: 1,
        href: `/ders-notlari/${uniSlug}/hukuk-felsefesi-donem-2`,
        ready: true,
        variantOf: 'hukuk-felsefesi',
      },
      {
        code: 'hukuk-felsefesi-yillik',
        title: 'Hukuk Felsefesi — Yıllık Tam Not',
        year: 1,
        href: `/ders-notlari/${uniSlug}/hukuk-felsefesi-yillik`,
        ready: true,
        variantOf: 'hukuk-felsefesi',
      },
    ];
...
console.log(
  `[hukuk-felsefesi-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${HUKUK_FELSEFESI_VARIANTS.join(',')}`
);
console.log(`[hukuk-felsefesi-triple] index notes now=${index.noteCount}`);
