/**
 * Tüm aktif fakülteler için İcra Hukuku 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-icra-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { ICRA_VARIANTS, buildIcraVariantNote } from './lib/icra-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of ICRA_VARIANTS) {
    const note = buildIcraVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(ICRA_VARIANTS);
...
index.icraTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'icra-donem-1',
        title: 'İcra Hukuku — 1. Dönem (Güz)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/icra-donem-1`,
        ready: true,
        variantOf: 'icra',
      },
      {
        code: 'icra-donem-2',
        title: 'İcra Hukuku — 2. Dönem (Bahar)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/icra-donem-2`,
        ready: true,
        variantOf: 'icra',
      },
      {
        code: 'icra-yillik',
        title: 'İcra Hukuku — Yıllık Tam Not',
        year: 4,
        href: `/ders-notlari/${uniSlug}/icra-yillik`,
        ready: true,
        variantOf: 'icra',
      },
    ];
...
console.log(
  `[icra-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${ICRA_VARIANTS.join(',')}`
);
console.log(`[icra-triple] index notes now=${index.noteCount}`);
