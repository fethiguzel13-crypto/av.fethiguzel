/**
 * Tüm aktif fakülteler için Kıymetli Evrak 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-kiymetli-evrak-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  KIYMETLI_EVRAK_VARIANTS,
  buildKiymetliEvrakVariantNote,
} from './lib/kiymetli-evrak-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of KIYMETLI_EVRAK_VARIANTS) {
    const note = buildKiymetliEvrakVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(KIYMETLI_EVRAK_VARIANTS);
...
index.kiymetliEvrakTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'kiymetli-evrak-donem-1',
        title: 'Kıymetli Evrak — 1. Dönem (Güz)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/kiymetli-evrak-donem-1`,
        ready: true,
        variantOf: 'kiymetli-evrak',
      },
      {
        code: 'kiymetli-evrak-donem-2',
        title: 'Kıymetli Evrak — 2. Dönem (Bahar)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/kiymetli-evrak-donem-2`,
        ready: true,
        variantOf: 'kiymetli-evrak',
      },
      {
        code: 'kiymetli-evrak-yillik',
        title: 'Kıymetli Evrak — Yıllık Tam Not',
        year: 4,
        href: `/ders-notlari/${uniSlug}/kiymetli-evrak-yillik`,
        ready: true,
        variantOf: 'kiymetli-evrak',
      },
    ];
...
console.log(
  `[ke-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${KIYMETLI_EVRAK_VARIANTS.join(',')}`
);
console.log(`[ke-triple] index notes now=${index.noteCount}`);
