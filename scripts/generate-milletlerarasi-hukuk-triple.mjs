/**
 * Tüm aktif fakülteler için Milletlerarası Hukuk 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-milletlerarasi-hukuk-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  MILLETLERARASI_HUKUK_VARIANTS,
  buildMilletlerarasiHukukVariantNote,
} from './lib/milletlerarasi-hukuk-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of MILLETLERARASI_HUKUK_VARIANTS) {
    const note = buildMilletlerarasiHukukVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(MILLETLERARASI_HUKUK_VARIANTS);
...
index.milletlerarasiHukukTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'milletlerarasi-hukuk-donem-1',
        title: 'Milletlerarası Hukuk — 1. Dönem (Güz)',
        year: 2,
        href: `/ders-notlari/${uniSlug}/milletlerarasi-hukuk-donem-1`,
        ready: true,
        variantOf: 'milletlerarasi-hukuk',
      },
      {
        code: 'milletlerarasi-hukuk-donem-2',
        title: 'Milletlerarası Hukuk — 2. Dönem (Bahar)',
        year: 2,
        href: `/ders-notlari/${uniSlug}/milletlerarasi-hukuk-donem-2`,
        ready: true,
        variantOf: 'milletlerarasi-hukuk',
      },
      {
        code: 'milletlerarasi-hukuk-yillik',
        title: 'Milletlerarası Hukuk — Yıllık Tam Not',
        year: 2,
        href: `/ders-notlari/${uniSlug}/milletlerarasi-hukuk-yillik`,
        ready: true,
        variantOf: 'milletlerarasi-hukuk',
      },
    ];
...
console.log(
  `[milletlerarasi-hukuk-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${MILLETLERARASI_HUKUK_VARIANTS.join(',')}`
);
console.log(`[milletlerarasi-hukuk-triple] index notes now=${index.noteCount}`);
