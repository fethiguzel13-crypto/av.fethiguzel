/**
 * Tüm aktif fakülteler için İdare Hukuku 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-idare-hukuku-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  IDARE_HUKUKU_VARIANTS,
  buildIdareHukukuVariantNote,
} from './lib/idare-hukuku-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of IDARE_HUKUKU_VARIANTS) {
    const note = buildIdareHukukuVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(IDARE_HUKUKU_VARIANTS);
...
index.idareHukukuTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'idare-hukuku-donem-1',
        title: 'İdare Hukuku — 1. Dönem (Güz)',
        year: 2,
        href: `/ders-notlari/${uniSlug}/idare-hukuku-donem-1`,
        ready: true,
        variantOf: 'idare-hukuku',
      },
      {
        code: 'idare-hukuku-donem-2',
        title: 'İdare Hukuku — 2. Dönem (Bahar)',
        year: 2,
        href: `/ders-notlari/${uniSlug}/idare-hukuku-donem-2`,
        ready: true,
        variantOf: 'idare-hukuku',
      },
      {
        code: 'idare-hukuku-yillik',
        title: 'İdare Hukuku — Yıllık Tam Not',
        year: 2,
        href: `/ders-notlari/${uniSlug}/idare-hukuku-yillik`,
        ready: true,
        variantOf: 'idare-hukuku',
      },
    ];
...
console.log(
  `[idare-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${IDARE_HUKUKU_VARIANTS.join(',')}`
);
console.log(`[idare-triple] index notes now=${index.noteCount}`);
