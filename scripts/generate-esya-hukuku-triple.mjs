/**
 * Tüm aktif fakülteler için Eşya Hukuku 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-esya-hukuku-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  ESYA_HUKUKU_VARIANTS,
  buildEsyaHukukuVariantNote,
} from './lib/esya-hukuku-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of ESYA_HUKUKU_VARIANTS) {
    const note = buildEsyaHukukuVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(ESYA_HUKUKU_VARIANTS);
...
index.esyaHukukuTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'esya-hukuku-donem-1',
        title: 'Eşya Hukuku — 1. Dönem (Güz)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/esya-hukuku-donem-1`,
        ready: true,
        variantOf: 'esya-hukuku',
      },
      {
        code: 'esya-hukuku-donem-2',
        title: 'Eşya Hukuku — 2. Dönem (Bahar)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/esya-hukuku-donem-2`,
        ready: true,
        variantOf: 'esya-hukuku',
      },
      {
        code: 'esya-hukuku-yillik',
        title: 'Eşya Hukuku — Yıllık Tam Not',
        year: 3,
        href: `/ders-notlari/${uniSlug}/esya-hukuku-yillik`,
        ready: true,
        variantOf: 'esya-hukuku',
      },
    ];
...
console.log(
  `[esya-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${ESYA_HUKUKU_VARIANTS.join(',')}`
);
console.log(`[esya-triple] index notes now=${index.noteCount}`);
