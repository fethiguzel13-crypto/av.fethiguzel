/**
 * Tüm aktif fakülteler için Deniz Ticareti 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-deniz-ticareti-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  DENIZ_TICARETI_VARIANTS,
  buildDenizTicaretiVariantNote,
} from './lib/deniz-ticareti-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of DENIZ_TICARETI_VARIANTS) {
    const note = buildDenizTicaretiVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(DENIZ_TICARETI_VARIANTS);
...
index.denizTicaretiTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'deniz-ticareti-donem-1',
        title: 'Deniz Ticareti — 1. Dönem (Güz)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/deniz-ticareti-donem-1`,
        ready: true,
        variantOf: 'deniz-ticareti',
      },
      {
        code: 'deniz-ticareti-donem-2',
        title: 'Deniz Ticareti — 2. Dönem (Bahar)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/deniz-ticareti-donem-2`,
        ready: true,
        variantOf: 'deniz-ticareti',
      },
      {
        code: 'deniz-ticareti-yillik',
        title: 'Deniz Ticareti — Yıllık Tam Not',
        year: 4,
        href: `/ders-notlari/${uniSlug}/deniz-ticareti-yillik`,
        ready: true,
        variantOf: 'deniz-ticareti',
      },
    ];
...
console.log(
  `[deniz-ticareti-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${DENIZ_TICARETI_VARIANTS.join(',')}`
);
console.log(`[deniz-ticareti-triple] index notes now=${index.noteCount}`);
