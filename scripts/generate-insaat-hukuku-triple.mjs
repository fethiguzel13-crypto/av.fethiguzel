/**
 * Tüm aktif fakülteler için İnşaat Hukuku 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-insaat-hukuku-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  INSAAT_HUKUKU_VARIANTS,
  buildInsaatHukukuVariantNote,
} from './lib/insaat-hukuku-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of INSAAT_HUKUKU_VARIANTS) {
    const note = buildInsaatHukukuVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(INSAAT_HUKUKU_VARIANTS);
...
index.insaatHukukuTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'insaat-hukuku-donem-1',
        title: 'İnşaat Hukuku — 1. Dönem (Güz)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/insaat-hukuku-donem-1`,
        ready: true,
        variantOf: 'insaat-hukuku',
      },
      {
        code: 'insaat-hukuku-donem-2',
        title: 'İnşaat Hukuku — 2. Dönem (Bahar)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/insaat-hukuku-donem-2`,
        ready: true,
        variantOf: 'insaat-hukuku',
      },
      {
        code: 'insaat-hukuku-yillik',
        title: 'İnşaat Hukuku — Yıllık Tam Not',
        year: 4,
        href: `/ders-notlari/${uniSlug}/insaat-hukuku-yillik`,
        ready: true,
        variantOf: 'insaat-hukuku',
      },
    ];
...
console.log(
  `[insaat-hukuku-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${INSAAT_HUKUKU_VARIANTS.join(',')}`
);
console.log(`[insaat-hukuku-triple] index notes now=${index.noteCount}`);
