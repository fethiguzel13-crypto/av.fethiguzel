/**
 * Tüm aktif fakülteler için İflas Hukuku 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-iflas-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { IFLAS_VARIANTS, buildIflasVariantNote } from './lib/iflas-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of IFLAS_VARIANTS) {
    const note = buildIflasVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(IFLAS_VARIANTS);
...
index.iflasTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'iflas-donem-1',
        title: 'İflas Hukuku — 1. Dönem (Güz)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/iflas-donem-1`,
        ready: true,
        variantOf: 'iflas',
      },
      {
        code: 'iflas-donem-2',
        title: 'İflas Hukuku — 2. Dönem (Bahar)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/iflas-donem-2`,
        ready: true,
        variantOf: 'iflas',
      },
      {
        code: 'iflas-yillik',
        title: 'İflas Hukuku — Yıllık Tam Not',
        year: 4,
        href: `/ders-notlari/${uniSlug}/iflas-yillik`,
        ready: true,
        variantOf: 'iflas',
      },
    ];
...
console.log(
  `[iflas-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${IFLAS_VARIANTS.join(',')}`
);
console.log(`[iflas-triple] index notes now=${index.noteCount}`);
