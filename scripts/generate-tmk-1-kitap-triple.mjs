/**
 * Tüm aktif fakülteler için TMK 1. Kitap 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-tmk-1-kitap-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  TMK_1_KITAP_VARIANTS,
  buildTmk1KitapVariantNote,
} from './lib/tmk-1-kitap-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of TMK_1_KITAP_VARIANTS) {
    const note = buildTmk1KitapVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(TMK_1_KITAP_VARIANTS);
...
index.tmk1KitapTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'tmk-1-kitap-donem-1',
        title: 'TMK 1. Kitap — 1. Dönem (Güz)',
        year: 1,
        href: `/ders-notlari/${uniSlug}/tmk-1-kitap-donem-1`,
        ready: true,
        variantOf: 'tmk-1-kitap',
      },
      {
        code: 'tmk-1-kitap-donem-2',
        title: 'TMK 1. Kitap — 2. Dönem (Bahar)',
        year: 1,
        href: `/ders-notlari/${uniSlug}/tmk-1-kitap-donem-2`,
        ready: true,
        variantOf: 'tmk-1-kitap',
      },
      {
        code: 'tmk-1-kitap-yillik',
        title: 'TMK 1. Kitap — Yıllık Tam Not',
        year: 1,
        href: `/ders-notlari/${uniSlug}/tmk-1-kitap-yillik`,
        ready: true,
        variantOf: 'tmk-1-kitap',
      },
    ];
...
console.log(
  `[tmk1-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${TMK_1_KITAP_VARIANTS.join(',')}`
);
console.log(`[tmk1-triple] index notes now=${index.noteCount}`);
