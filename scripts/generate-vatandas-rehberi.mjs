/**
 * Generates lib/vatandas-rehberi/data.ts — SEO citizen guides (target: 100).
...
function genericBody(t) {
  ...
  const lead = `${t.h1.replace(/\?$/, '')} konusu, Türkiye’de vatandaşların en sık aradığı hukuki ve idari sorulardan biridir. Bu rehber «${k0}» ve «${k1}» aramalarına yanıt verecek şekilde; tanımlar, adımlar, belgeler, süreler ve sık hatalar üzerinden genel bilgilendirme sunar. Metin bağlayıcı hukuki tavsiye yerine geçmez; somut olayda mevzuat, tebliğ tarihi ve avukat değerlendirmesi esas alınmalıdır.`;

  const sections = [
    s(`${t.category} bağlamında konu nedir?`, [
      `${t.h1} sorusu, ${t.category.toLocaleLowerCase('tr-TR')} alanında hak ve yükümlülüklerin doğru teşhis edilmesiyle başlar. Yanlış mercie başvuru veya süre kaçırma, en sık görülen hak kaybı sebepleridir.`,
      `Arama motorlarında «${k0}» yazan kullanıcılar genellikle hem tanımı hem de «ne yapmalıyım?» yol haritasını arar. Bu nedenle aşağıdaki bölümler tanım, süreç, belge ve riskleri birlikte ele alır.`,
    ], t.keywords.slice(0, 4)),
    s('Kimler ilgilidir? Hangi şartlar aranır?', [
  ...
  const steps = [
    `«${k0}» ile ilgili tüm tebliğ, sözleşme ve belgeleri bir klasörde toplayın.`,
  ...
  const faqList = [
    faq(`${k0} için avukat zorunlu mu?`, 'Çoğu başvuruda avukat zorunlu değildir; ancak süre ve usul hataları hak kaybına yol açabileceğinden karmaşık dosyalarda hukuki destek önerilir.'),
    faq('İşlem ne kadar sürer?', 'Kurum ve mahkeme yoğunluğu, delil durumu ve itirazlara göre değişir. Kesin süre vaadi verilemez.'),
    faq('e-Devlet yeterli midir?', 'Birçok işlem e-Devlet ile başlar veya tamamlanır; bazı merciler hâlâ fiziki evrak veya randevu ister.'),
    faq('Bu rehber bağlayıcı mıdır?', 'Hayır. Genel bilgilendirmedir; yürürlükteki mevzuat ve somut olay esastır.'),
  ];
