/**
 * Tüm aktif fakülteler için TMK 2. Kitap 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-tmk-2-kitap-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  TMK_2_KITAP_VARIANTS,
  buildTmk2KitapVariantNote,
} from './lib/tmk-2-kitap-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of TMK_2_KITAP_VARIANTS) {
    const note = buildTmk2KitapVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(TMK_2_KITAP_VARIANTS);
...
index.tmk2KitapTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'tmk-2-kitap-donem-1',
        title: 'TMK 2. Kitap — 1. Dönem (Güz)',
        year: 2,
        href: `/ders-notlari/${uniSlug}/tmk-2-kitap-donem-1`,
        ready: true,
        variantOf: 'tmk-2-kitap',
      },
      {
        code: 'tmk-2-kitap-donem-2',
        title: 'TMK 2. Kitap — 2. Dönem (Bahar)',
        year: 2,
        href: `/ders-notlari/${uniSlug}/tmk-2-kitap-donem-2`,
        ready: true,
        variantOf: 'tmk-2-kitap',
      },
      {
        code: 'tmk-2-kitap-yillik',
        title: 'TMK 2. Kitap — Yıllık Tam Not',
        year: 2,
        href: `/ders-notlari/${uniSlug}/tmk-2-kitap-yillik`,
        ready: true,
        variantOf: 'tmk-2-kitap',
      },
    ];
...
console.log(
  `[tmk2-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${TMK_2_KITAP_VARIANTS.join(',')}`
);
console.log(`[tmk2-triple] index notes now=${index.noteCount}`);
