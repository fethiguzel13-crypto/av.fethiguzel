...
for (const a of articles) {
  ...
  // SEO title/description: kısa başlık ve desc’i güçlendir (SERP + E-E-A-T)
  if (!a.title || a.title.length < 28) {
    a.title = `${a.h1.replace(/\?$/, '')} — Vatandaş Rehberi | Av. Fethi Güzel`.slice(0, 70);
  } else if (!/fethi|güzel|rehber|nedir|nasıl/i.test(a.title) && a.title.length < 45) {
    a.title = `${a.title} | Vatandaş Rehberi`.slice(0, 70);
  }
  if (!a.description || a.description.length < 90) {
    a.description =
      `${a.h1} hakkında adım adım bilgilendirme: süre, merci, belge ve riskler. ` +
      `Genel bilgilendirme — Av. Fethi Güzel Hukuk Portalı. Somut dosyada avukata danışın.`;
    if (a.description.length > 160) a.description = a.description.slice(0, 157) + '…';
  }
}

// —— İki yönlü iç link: her sayfaya en az 2 inbound (crawl) ——
...
function countInbound() {
  const map = new Map(articles.map((a) => [a.slug, 0]));
  for (const a of articles) {
...
const pillarsList = articles.filter((a) => a.role === 'pillar');
// 5 tur: her turda 0–1 inbound kalanlara donor ekle
for (let pass = 0; pass < 5; pass++) {
  const inbound = countInbound();
  for (const a of articles) {
    const cur = inbound.get(a.slug) || 0;
    if (cur >= 2) continue;
    const peers = (byCategory.get(a.category) || []).filter((x) => x.slug !== a.slug);
    const donors = [
      pillarsList.find((p) => p.category === a.category),
      peers[0],
      peers[1],
      peers[2],
      peers[Math.abs(hashSlug(a.slug)) % Math.max(peers.length, 1)],
      pillarsList[Math.abs(hashSlug(a.slug)) % Math.max(pillarsList.length, 1)],
      pillarsList[(Math.abs(hashSlug(a.slug)) + 1) % Math.max(pillarsList.length, 1)],
      articles[(Math.abs(hashSlug(a.slug + 'x')) + pass) % articles.length],
      articles[(Math.abs(hashSlug(a.slug + 'y')) + pass * 3) % articles.length],
    ].filter(Boolean);
    let need = 2 - cur;
    for (const d of donors) {
      if (need <= 0) break;
      if (d.slug === a.slug) continue;
      if ((d.related || []).includes(a.slug)) continue;
      // limit doluysa en eski spoke’u düş
      if ((d.related || []).length >= 12) {
        d.related = (d.related || []).slice(1);
      }
      d.related = [...(d.related || []), a.slug];
      need--;
    }
  }
}
// Mutlak kapanış: hâlâ 0 ise hub-spoke sabit bağ
{
  const inbound = countInbound();
  const hub0 = pillarsList[0] || articles[0];
  const hub1 = pillarsList[1] || articles[1] || hub0;
  for (const a of articles) {
    if ((inbound.get(a.slug) || 0) > 0) continue;
    if (hub0 && hub0.slug !== a.slug && !(hub0.related || []).includes(a.slug)) {
      hub0.related = [...(hub0.related || []).slice(-10), a.slug];
    }
    if (hub1 && hub1.slug !== a.slug && !(hub1.related || []).includes(a.slug)) {
      hub1.related = [...(hub1.related || []).slice(-10), a.slug];
    }
  }
}
