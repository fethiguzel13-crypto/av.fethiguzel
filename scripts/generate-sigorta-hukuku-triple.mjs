/**
 * Tüm aktif fakülteler için Sigorta Hukuku 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-sigorta-hukuku-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  SIGORTA_HUKUKU_VARIANTS,
  buildSigortaHukukuVariantNote,
} from './lib/sigorta-hukuku-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of SIGORTA_HUKUKU_VARIANTS) {
    const note = buildSigortaHukukuVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(SIGORTA_HUKUKU_VARIANTS);
...
index.sigortaHukukuTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'sigorta-hukuku-donem-1',
        title: 'Sigorta Hukuku — 1. Dönem (Güz)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/sigorta-hukuku-donem-1`,
        ready: true,
        variantOf: 'sigorta-hukuku',
      },
      {
        code: 'sigorta-hukuku-donem-2',
        title: 'Sigorta Hukuku — 2. Dönem (Bahar)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/sigorta-hukuku-donem-2`,
        ready: true,
        variantOf: 'sigorta-hukuku',
      },
      {
        code: 'sigorta-hukuku-yillik',
        title: 'Sigorta Hukuku — Yıllık Tam Not',
        year: 4,
        href: `/ders-notlari/${uniSlug}/sigorta-hukuku-yillik`,
        ready: true,
        variantOf: 'sigorta-hukuku',
      },
    ];
...
console.log(
  `[sigorta-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${SIGORTA_HUKUKU_VARIANTS.join(',')}`
);
console.log(`[sigorta-triple] index notes now=${index.noteCount}`);
