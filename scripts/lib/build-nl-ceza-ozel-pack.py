...
OUT = ROOT / "docs" / "ders-notlari" / "notebooklm" / "dalga-a-ceza-ozel"
GRAPH = ROOT / "lib" / "ders-notlari" / "graphs" / "ceza-ozel.json"
...
def main() -> None:
    ...
    parts = ["# Ceza özel — resmi lafız (FSEK m. 31)\n", "Şerh yoktur. TCK özel hükümler.\n"]
    seen = set()
    ...
    (OUT / "tck-ozel-lafiz.md").write_text("\n".join(parts), encoding="utf-8")
    lines = [
        f"# Graf: {g['title']}",
    ...
    (OUT / "harita.md").write_text(
        """# Dalga A — Ceza hukuku özel hükümler

Quiz TCK lafzı ve graf unsura dayanır. Künye uydurulmaz. Slayt yoktur.

Kurum id: kasten-oldurme, kasten-yaralama, hakaret, hirizlik, dolandiricilik, uyusturucu, zimmet, gorevi-kotuye.

Tuzaklar: 81/82/83; 86/87; hakaret/sovme; basit/nitelikli hırsızlık; dolandırıcılık/hırsızlık; 188/191; zimmet/görevi kötüye kullanma.

Fakülte sesi overlay'dedir. Ceza genel (tipiklik/kanunilik) bu kâğıtta asıl kapı değildir.
""",
        encoding="utf-8",
    )
