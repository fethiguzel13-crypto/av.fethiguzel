...
test('gerçek paketlerde bilinen sonuçlar korunur', { skip: !existsSync('content-packs/tmk.json.gz') }, () => {
  ...
  // TMK 166 artık gerçek maddeye özgü yorum içeriyor, yayınlanabilir olmalı
  const r166 = auditCommentary('tmk', tmk['madde-166']?.commentary || '');
  assert.equal(r166.publishable, true, `TMK 166 yayınlanabilir olmalıydı (${r166.verdict})`);
});
