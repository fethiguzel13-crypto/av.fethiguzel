/**
 * Tüm aktif fakülteler için Borçlar Özel 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-borclar-ozel-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  BORCLAR_OZEL_VARIANTS,
  buildBorclarOzelVariantNote,
} from './lib/borclar-ozel-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of BORCLAR_OZEL_VARIANTS) {
    const note = buildBorclarOzelVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(BORCLAR_OZEL_VARIANTS);
...
index.borclarOzelTripleAt = new Date().toISOString();
    ...
    const extras = [
      {
        ...
        title: 'Borçlar Özel — 1. Dönem (Güz)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/borclar-ozel-donem-1`,
        ready: true,
        variantOf: 'borclar-ozel',
      },
      {
        code: 'borclar-ozel-donem-2',
        title: 'Borçlar Özel — 2. Dönem (Bahar)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/borclar-ozel-donem-2`,
        ready: true,
        variantOf: 'borclar-ozel',
      },
      {
        code: 'borclar-ozel-yillik',
        title: 'Borçlar Özel — Yıllık Tam Not',
        year: 3,
        href: `/ders-notlari/${uniSlug}/borclar-ozel-yillik`,
        ready: true,
        variantOf: 'borclar-ozel',
      },
    ];
...
console.log(
  `[borclar-ozel-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${BORCLAR_OZEL_VARIANTS.join(',')}`
);
console.log(`[borclar-ozel-triple] index notes now=${index.noteCount}`);
