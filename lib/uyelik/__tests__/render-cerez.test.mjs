...
test('oturum çerezi tazelemesi route handler içinde duruyor', () => {
  const p = join(process.cwd(), 'lib', 'uyelik', 'http.ts');
  const src = yorumsuz(readFileSync(p, 'utf8'));
  assert.match(
    src,
    /setSessionCookie\s*\(/,
    'Tazeleme üyelik API dağıtıcısından kaldırılmış — çerez artık hiç yenilenmiyor'
  );
  assert.match(src, /key === 'ben'/, 'GET ben yolu dağıtıcıda durmalı');
});
