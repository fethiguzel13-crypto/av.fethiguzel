...
function Card({ id, icon, title, tag, children, fav, onToggleFav, onOpen }: {
  ...
  const meta = META_ARACLAR.find(m => m.id === id);
  const kavram = getKavramForHesaplama(id);
    ...
    <div id={id} className={`bg-white border rounded-2xl sm:rounded-[2rem] overflow-hidden shadow-sm transition-colors ${open ? "border-accent/25 shadow-md" : "border-charcoal/6"}`}>
      <div className="flex items-stretch">
        ...
        {onToggleFav && (
          ...
          >
            <Star size={18} className={fav ? "fill-accent text-accent" : ""} />
          </button>
        )}
      </div>
      {open && (
          ...
          <div className="mb-4 flex flex-wrap items-center justify-end gap-3">
            {kavram && (
              <Link
                ...
                className="text-[10px] font-bold uppercase tracking-wider text-charcoal/35 hover:text-accent transition-colors"
            ...
            <Link
              href={`/hesaplama/${id}`}
              className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-charcoal/35 hover:text-accent transition-colors"
              onClick={(e) => e.stopPropagation()}
            >
              Ayrı sayfa <ExternalLink size={11} />
            </Link>
          </div>
          ...
          {meta?.ilgili && meta.ilgili.length > 0 && (
            <div className="mt-5 pt-4 border-t border-charcoal/6">
              ...
              <div className="flex flex-wrap gap-1.5">
                {meta.ilgili.map(rid => {
                  const m = META_ARACLAR.find(x => x.id === rid);
                  return (
                    <a
                      key={rid}
                      href={`#${rid}`}
                      onClick={() => {
                        ...
                        if (el) {
                          ...
                          // hash ile açılması için
                          window.location.hash = rid;
                        }
                      }}
                      className="text-[11px] px-2.5 py-1 rounded-full bg-charcoal/5 text-charcoal/60 hover:bg-accent/10 hover:text-accent"
                    >
                      {m?.baslik ?? rid}
                    </a>
                  );
...
function buildResultText(baslikMetin: string, rows: [string, string][], note?: string) {
  ...
  return [
    ...
    ...rows.map(([k, v]) => `${k}: ${v}`),
    note ? `\nNot: ${note}` : "",
    "\nKaynak: Av. Fethi Güzel — Hukuki Hesaplama (bilgi amaçlıdır, garanti değildir)",
    "https://avfethiguzel.com/hesaplama",
  ].filter(Boolean).join("\n");
}
...
function Result({ rows, note, baslik }: { rows: [string, string][]; note?: string; baslik?: string }) {
  ...
  const yazdir = () => {
    ...
    const html = `<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${baslikMetin}</title></head>`
      ...
      + `<div style="font-size:12px;color:#999;margin-bottom:18px;border-bottom:1px solid #eee;padding-bottom:10px">Av. Fethi Güzel — Hukuki Hesaplama · ${tarih}</div>`
      ...
      + `<p style="font-size:10px;color:#bbb;margin-top:28px">Bu çıktı bilgi amaçlıdır · Kesin sonuç için avukatınıza danışın · avfethiguzel.com/hesaplama</p>`
      + `</body></html>`;
  ...
  return (
    <div className="mt-5 bg-primary/5 border border-primary/10 rounded-xl sm:rounded-2xl p-4 sm:p-5">
      {rows.length > 0 && (
        <div className="flex flex-wrap justify-end gap-3 -mt-1 mb-1.5">
          <button
            type="button"
            onClick={kopyala}
...
/**
 * Ana hesaplama sayfası — tüm araçlar tam boyutta, açılır kartlarda.
 * Ayrı SEO sayfaları (/hesaplama/[slug]) ayrıca durur; burası küçültülmez.
 */
export default function HesaplamaHub() {
  ...
  useEffect(() => {
    const t = window.setTimeout(() => {
      try {
        ...
        if (aktifTag) sp.set("tag", aktifTag);
        const qs = sp.toString();
        const next = qs ? `/hesaplama?${qs}` : "/hesaplama";
  ...
  const tumTagler = useMemo(() => Array.from(new Set(ARACLAR.map(a => a.tag))), []);

  const metaById = useMemo(() => {
    const m = new Map(META_ARACLAR.map(x => [x.id, x]));
    return m;
  }, []);

  const filtrelenmis = useMemo(() => {
    return ARACLAR.filter(a => {
      if (sadeceFav && !favs.includes(a.id)) return false;
      const meta = metaById.get(a.id);
      const aramaUyar = matchHesaplamaTool(
        {
          id: a.id,
          baslik: a.baslik,
          tag: a.tag,
  ...
  }, [arama, aktifTag, sadeceFav, favs, metaById]);
  ...
  const recentTools = recent
    .map(id => ARACLAR.find(a => a.id === id))
    .filter((x): x is (typeof ARACLAR)[0] => Boolean(x))
    .slice(0, 6);
      ...
      <main className="min-h-screen bg-cream pt-24 sm:pt-32 md:pt-36 pb-16 sm:pb-24 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8 sm:mb-12">
            ...
            <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-sm text-charcoal/50">
              <Link href="/kavram" className="text-accent font-semibold hover:underline">Kavramlar</Link>
              <Link href="/ara" className="text-accent font-semibold hover:underline">Mevzuatta ara</Link>
              <Link href="/tarife-guncellemeleri" className="hover:text-accent">Tarife güncellemeleri</Link>
            </div>
          </div>
          ...
          <div className="mb-6 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            {[
              { k: "Araç", v: String(ARACLAR.length) },
              { k: "Kategori", v: String(tumTagler.length) },
          ...
          {(favTools.length > 0 || recentTools.length > 0) && !arama && !aktifTag && !sadeceFav && (
          ...
          <div className="mb-5 sm:mb-7 sticky top-20 z-20 bg-cream/95 backdrop-blur-sm py-3 -mx-1 px-1 rounded-2xl">
            <div className="relative mb-3">
              ...
              <input
                ...
                placeholder="Akıllı ara — kıdem, satım, faiz, ihbar, yatar, harç…"
                aria-label="Hesaplama aracı ara"
                className="w-full border border-charcoal/15 rounded-full pl-10 pr-4 py-2.5 sm:py-3 text-sm text-charcoal bg-white focus:outline-none focus:border-accent transition-colors shadow-sm"
              />
            </div>
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              <button
                type="button"
                onClick={() => { setAktifTag(null); setSadeceFav(false); }}
                className={`text-[10px] sm:text-[11px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-full transition-colors ${!aktifTag && !sadeceFav ? "bg-charcoal text-cream" : "bg-charcoal/5 text-charcoal/45 hover:bg-charcoal/10"}`}
              >
                Tümü ({ARACLAR.length})
              </button>
              <button
                type="button"
                onClick={() => { setSadeceFav(v => !v); setAktifTag(null); }}
                className={`inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-full transition-colors ${sadeceFav ? "bg-accent text-white" : "bg-charcoal/5 text-charcoal/45 hover:bg-charcoal/10"}`}
              >
                <Star size={11} className={sadeceFav ? "fill-white" : ""} /> Favori ({favs.length})
              </button>
              {tumTagler.map(t => (
                <button
                  key={t}
                  type="button"
                  onClick={() => { setAktifTag(aktifTag === t ? null : t); setSadeceFav(false); }}
            ...
            {(arama || aktifTag || sadeceFav) && (
              <p className="mt-2 text-[11px] text-charcoal/40">
                {filtrelenmis.length} araç gösteriliyor
                {arama ? ` · “${arama.trim()}”` : ""}
                {aktifTag ? ` · ${aktifTag}` : ""}
                {" · "}
                <button type="button" className="text-accent font-semibold hover:underline" onClick={() => { setArama(""); setAktifTag(null); setSadeceFav(false); }}>
                  Filtreyi temizle
                </button>
              </p>
            )}
          ...
          {/* Araçlar — tam hesaplayıcılar, açılır kartlar */}
          <div className="flex flex-col gap-3 sm:gap-4">
            ...
            {filtrelenmis.length === 0 && (
              <div className="text-center py-10 text-sm text-charcoal/40 space-y-2">
                <p>Aramanızla eşleşen araç bulunamadı.</p>
                <p className="text-xs">Deneyin: kıdem, faiz, miras, kira, harç, infaz, nafaka</p>
                <button
                  type="button"
                  onClick={() => { setArama(""); setAktifTag(null); setSadeceFav(false); }}
          ...
          <div className="mt-10 sm:mt-16 bg-charcoal rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-10 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <Link
                href="/#iletisim"
                className="inline-block bg-cream/10 text-cream px-6 sm:px-8 py-3 sm:py-3.5 rounded-full font-bold text-sm hover:bg-cream/20 transition-colors"
              >
