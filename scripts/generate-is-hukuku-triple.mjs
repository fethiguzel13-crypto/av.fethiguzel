/**
 * Tüm aktif fakülteler için İş Hukuku 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-is-hukuku-triple.mjs
 */
...
import {
  IS_HUKUKU_VARIANTS,
  buildIsHukukuVariantNote,
} from './lib/is-hukuku-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of IS_HUKUKU_VARIANTS) {
    const note = buildIsHukukuVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(IS_HUKUKU_VARIANTS);
...
index.isHukukuTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'is-hukuku-donem-1',
        title: 'İş Hukuku — 1. Dönem (Güz)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/is-hukuku-donem-1`,
        ready: true,
        variantOf: 'is-hukuku',
      },
      {
        code: 'is-hukuku-donem-2',
        title: 'İş Hukuku — 2. Dönem (Bahar)',
        year: 3,
        href: `/ders-notlari/${uniSlug}/is-hukuku-donem-2`,
        ready: true,
        variantOf: 'is-hukuku',
      },
      {
        code: 'is-hukuku-yillik',
        title: 'İş Hukuku — Yıllık Tam Not',
        year: 3,
        href: `/ders-notlari/${uniSlug}/is-hukuku-yillik`,
        ready: true,
        variantOf: 'is-hukuku',
      },
    ];
...
console.log(
  `[is-hukuku-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${IS_HUKUKU_VARIANTS.join(',')}`
);
console.log(`[is-hukuku-triple] index notes now=${index.noteCount}`);
