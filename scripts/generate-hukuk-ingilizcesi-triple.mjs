/**
 * Tüm aktif fakülteler için Hukuk İngilizcesi 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-hukuk-ingilizcesi-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  HUKUK_INGILIZCESI_VARIANTS,
  buildHukukIngilizcesiVariantNote,
} from './lib/hukuk-ingilizcesi-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of HUKUK_INGILIZCESI_VARIANTS) {
    const note = buildHukukIngilizcesiVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(HUKUK_INGILIZCESI_VARIANTS);
...
index.hukukIngilizcesiTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'hukuk-ingilizcesi-donem-1',
        title: 'Hukuk İngilizcesi — 1. Dönem (Güz)',
        year: 1,
        href: `/ders-notlari/${uniSlug}/hukuk-ingilizcesi-donem-1`,
        ready: true,
        variantOf: 'hukuk-ingilizcesi',
      },
      {
        code: 'hukuk-ingilizcesi-donem-2',
        title: 'Hukuk İngilizcesi — 2. Dönem (Bahar)',
        year: 1,
        href: `/ders-notlari/${uniSlug}/hukuk-ingilizcesi-donem-2`,
        ready: true,
        variantOf: 'hukuk-ingilizcesi',
      },
      {
        code: 'hukuk-ingilizcesi-yillik',
        title: 'Hukuk İngilizcesi — Yıllık Tam Not',
        year: 1,
        href: `/ders-notlari/${uniSlug}/hukuk-ingilizcesi-yillik`,
        ready: true,
        variantOf: 'hukuk-ingilizcesi',
      },
    ];
...
console.log(
  `[hukuk-ingilizcesi-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${HUKUK_INGILIZCESI_VARIANTS.join(',')}`
);
console.log(`[hukuk-ingilizcesi-triple] index notes now=${index.noteCount}`);
