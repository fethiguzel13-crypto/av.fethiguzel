...
OUT = ROOT / "docs" / "ders-notlari" / "notebooklm" / "dalga-a-kiymetli-evrak"
GRAPH = ROOT / "lib" / "ders-notlari" / "graphs" / "kiymetli-evrak.json"
...
def main() -> None:
    ...
    parts = ["# Kıymetli evrak — resmi lafız (FSEK m. 31)\n", "Şerh yoktur. TTK + ÇekK.\n"]
    seen = set()
    ...
    (OUT / "harita.md").write_text(
        """# Dalga A — Kıymetli evrak

Quiz TTK/ÇekK lafzı ve graf unsura dayanır. Künye uydurulmaz. Slayt yoktur.

Kurum id: ke-kavram, polise, bono, cir-teslim, cek, defi, basvuru, karsiliksiz.

Tuzaklar: nama/emre/hamiline; poliçe/bono; ciro/teslim; çek unsuru; def'i; başvuru; karşılıksız çek.

Fakülte sesi overlay'dedir. Ticari işletme ve şirketler ayrı derstir.
""",
        encoding="utf-8",
    )
