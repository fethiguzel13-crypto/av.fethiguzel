/**
 * Tüm aktif fakülteler için Arabuluculuk 1. dönem / 2. dönem / yıllık.
 * Run: node scripts/generate-arabuluculuk-triple.mjs
 */
import { writeFileSync, mkdirSync, readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  ARABULUCULUK_VARIANTS,
  buildArabuluculukVariantNote,
} from './lib/arabuluculuk-triple.mjs';
...
for (const uni of activeUnis) {
  for (const v of ARABULUCULUK_VARIANTS) {
    const note = buildArabuluculukVariantNote(uni, v);
    newNotes.push(note);
...
const dropCodes = new Set(ARABULUCULUK_VARIANTS);
...
index.arabuluculukTripleAt = new Date().toISOString();
...
if (existsSync(hubsPath)) {
  ...
  for (const hub of hubs) {
    ...
    const extras = [
      {
        code: 'arabuluculuk-donem-1',
        title: 'Arabuluculuk — 1. Dönem (Güz)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/arabuluculuk-donem-1`,
        ready: true,
        variantOf: 'arabuluculuk',
      },
      {
        code: 'arabuluculuk-donem-2',
        title: 'Arabuluculuk — 2. Dönem (Bahar)',
        year: 4,
        href: `/ders-notlari/${uniSlug}/arabuluculuk-donem-2`,
        ready: true,
        variantOf: 'arabuluculuk',
      },
      {
        code: 'arabuluculuk-yillik',
        title: 'Arabuluculuk — Yıllık Tam Not',
        year: 4,
        href: `/ders-notlari/${uniSlug}/arabuluculuk-yillik`,
        ready: true,
        variantOf: 'arabuluculuk',
      },
    ];
...
console.log(
  `[arabuluculuk-triple] unis=${activeUnis.length} notes=${newNotes.length} variants=${ARABULUCULUK_VARIANTS.join(',')}`
);
console.log(`[arabuluculuk-triple] index notes now=${index.noteCount}`);
